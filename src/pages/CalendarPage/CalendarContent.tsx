...
const CalendarContent: React.FC<Props> = ({ showTitle = false }) => {
  ...
  return (
    <>
      {showTitle && <h1 className={s.title}>Календарь курсов</h1>}
      ...
      {store.selectedEvent && (
        <div className={s.overlay} onClick={closeSelectedEvent}>
          <div className={s.popup} onClick={(e) => e.stopPropagation()}>
            <button type="button" className={s.popupClose} aria-label="Закрыть" onClick={closeSelectedEvent}>
              Г—
            </button>
            <div
            ...
            <div className={s.popupDetails}>
              ...
              <div className={s.popupRow}>
                ...
                <span>
                  {format(store.selectedEvent.start, 'HH:mm')} – {format(store.selectedEvent.end, 'HH:mm')}
                </span>
              </div>
            </div>
